...
export default function ResultsPage() {
  ...
  const [currencyValue, setCurrencyValue] = useState(1.0);
  ...
  const { get } = useFetch(
    "https://v6.exchangerate-api.com/v6/b560af4e412f93257f414644/pair"
  );
  useEffect(() => {
    get(`/${baseCurrCode}/${targetCurrCode}`).then((data) => {
      console.log(data);
  ...
  return (
    <>
      <div className="container">
        <h2 className="center-text company-logo header">
          Current<span className="blue-text">Currency</span> Converter
        </h2>
        <p className="currency-paragraph">
          Convert live foreign currency exchange rates
        </p>
        <CurrencyInput
          onValueChange={(value) => handleValueChange(value)}
          className="input-currency big-blue-text"
        />
  ...
  if it's false then it does the orginal base/target */}
        <div className="result-card-flex">
          <div className="selected-currency-card">
            <Twemoji text={swap ? baseFlagEmoji : targetFlagEmoji} />{" "}
            {swap ? baseCurrencyCode : targetCurrencyCode}
            <span className="text-dimmed">
              {" "}
              - {swap ? baseCurrName : targetCurrName}
            </span>
          </div>
          <button
            id="swap-base-btn"
            onClick={() => handleBaseChange()}
            className="ax-button rounded-swap-button"
          >
            <img
              src="/assets/swap.svg"
              alt="swap button"
          ...
          <div className="selected-currency-card">
            <Twemoji text={swap ? targetFlagEmoji : baseFlagEmoji} />{" "}
            {swap ? targetCurrencyCode : baseCurrencyCode}
            <span className="text-dimmed">
              {" "}
              - {swap ? targetCurrName : baseCurrName}
            </span>
          </div>
        </div>
        <div className="conversion-flex center-text">
          <p className="text-dimmed conv-upper">
            {currencyValue} {swap ? baseCurrName : targetCurrName} =
          </p>
          <p className="conv-lower">
            {" "}
            {swap
              ? Number(currencyValue * conversionRate).toFixed(4)
              : Number(currencyValue / conversionRate).toFixed(4)}{" "}
            {swap ? targetCurrName : baseCurrName}
          </p>
        </div>
        <div className="results-update-flex">
          <p>
            <span className="dimmed-underlined">
              {swap ? baseCurrName : targetCurrName}
            </span>{" "}
            to{" "}
            <span className="dimmed-underlined">
          ...
          <button
            ...
            className="rounded-button ax-button"
          >
            Refresh
          </button>
        </div>
